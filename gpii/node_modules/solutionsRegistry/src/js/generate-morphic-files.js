...
gpii.solutionsRegistry.morphicFileGenerator.resolveSimpleTransformArgument = function (transform, key) {
    if (transform[key] !== undefined && fluid.isPrimitive(transform[key])) {
        return transform[key];
    } else {
        var path = transform[key + "Path"];
        if (path) {
            return "${" + path + "}";
        }
    }
...
gpii.solutionsRegistry.morphicFileGenerator.rewriteSimpleTransform = function (transform) {
    if (transform && transform.type) {
        if (transform.type === "fluid.transforms.linearScale") {
            var input = gpii.solutionsRegistry.morphicFileGenerator.resolveSimpleTransformArgument(transform, "input");
            if (input !== undefined) {
                return {
                    simpleTransform: transform.factor + " * " + input + " + " + (transform.offset || 0)
                };
            }
        } else if (transform.type === "fluid.transforms.binaryOp") {
            var left = gpii.solutionsRegistry.morphicFileGenerator.resolveSimpleTransformArgument(transform, "left");
            var right = gpii.solutionsRegistry.morphicFileGenerator.resolveSimpleTransformArgument(transform, "right");
            if (left !== undefined && right !== undefined) {
                return {
                    simpleTransform: left + " " + transform.operator + right
                };
            }
        }
    }
...
gpii.solutionsRegistry.morphicFileGenerator.rewriteSimpleTransformsBlock = function (solutionTranslations, path) {
    var block = fluid.get(solutionTranslations, path);
    fluid.each(block, function (transform, key) {
        var rewrite = gpii.solutionsRegistry.morphicFileGenerator.rewriteSimpleTransform(transform.transform);
        if (rewrite) {
            delete block[key];
            block[key] = rewrite;
        }
...
gpii.solutionsRegistry.morphicFileGenerator.rewriteSimpleTransformsAll = function (solutionTranslations) {
    var togo = fluid.copy(solutionTranslations);
    fluid.each(togo, function (value, key) {
        gpii.solutionsRegistry.morphicFileGenerator.rewriteSimpleTransformsBlock(togo, [key, "capabilitiesTransformations"]);
        gpii.solutionsRegistry.morphicFileGenerator.rewriteSimpleTransformsBlock(togo, [key, "inverseCapabilitiesTransformations"]);
    });
    return togo;
};
...
gpii.solutionsRegistry.morphicFileGenerator.generate = function (that) {
    ...
    fluid.each(that.options.fileDefs, function (contextFileGrade, contextFileKey) {
        ...
        fluid.each(contextSolutionComponents, function (solutionComponent) {
            ...
            console.log("Writing file " + translationsPath);
            var simplifiedTranslations = gpii.solutionsRegistry.morphicFileGenerator.rewriteSimpleTransformsAll(solutionTranslations);
            fs.writeFileSync(translationsPath, JSON.stringify(simplifiedTranslations, null, 4) + "\n", { encoding: "utf8"});
        });
