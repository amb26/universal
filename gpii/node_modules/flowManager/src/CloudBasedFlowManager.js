...
fluid.defaults("gpii.flowManager.cloudBased.oauth2", {
    ...
    components: {
        ...
        pingServer: {
            type: "gpii.flowManager.pingServer"
        },
        userService: {
...
fluid.defaults("gpii.flowManager.cloudBased.oauth2settings.handler", {
    ...
    invokers: {
        handleRequest: {
            ...
            args: ["{that}", "{gpii.flowManager.cloudBased.oauth2}.oauth2DataStore"]
        },
        matchToSettings: {
            funcName: "gpii.flowManager.cloudBased.matchToSettings",
            args: ["{arguments}.0", "{request}.events.onSuccess"]
...
gpii.flowManager.cloudBased.oauth2settings.handleRequest = function (request, dataStore) {
    ...
    if (!accessToken) {
        ...
        authPromise.then(function (auth) {
            if (!auth) {
                ...
                request.events.onUserToken.fire(auth.userGpiiToken);
                request.events.onDeviceContext.fire(gpii.flowManager.cloudBased.solutionToDeviceContext(auth.oauth2ClientId));
            }
        }, function () {
            gpii.oauth2.sendUnauthorized(request);
        });
